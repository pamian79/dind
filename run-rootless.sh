...
DOCKERD_DATA_ROOT=${DOCKERD_DATA_ROOT:-/home/rootless/var/lib/docker}
...
if [[ $(id -u) != "0" ]]; then
  ROOTLESS=true
  DOCKERD_PID_FILE=/var/run/user/$(id -u)/docker.pid
  DOCKERD_SOCK_FILE=/var/run/user/$(id -u)/docker.sock
else
  DOCKERD_PID_FILE=/var/run/docker.pid
  DOCKERD_SOCK_FILE=/var/run/docker.sock
fi
...
do
  ...
  if [[ -z "${ROOTLESS}" ]]; then
    ...
    DOCKERD_PORT=${DOCKERD_PORT:-1300}
    rootlesskit --net=vpnkit --mtu=1500 --disable-host-loopback --port-driver=builtin \
      --copy-up=/etc --copy-up=/run \
      -p 0.0.0.0:${DOCKERD_PORT}:${DOCKERD_PORT}/tcp \
      dockerd ${DOCKERD_PARAMS} --config-file /etc/docker/daemon.json <&- &
  fi
  ...
  if [[ -n "${ROOTLESS}" ]]; then
    ln -sfv "${DOCKERD_SOCK_FILE}" /var/run/docker.sock
  fi
  CNT=0
  while ! docker ps
  do
...
if [[ -z "${ROOTLESS}" ]]; then
  ...
  ROOTLESSKIT_PID=$(ps -o pid,ppid,comm | awk '$3 == "rootlesskit"{print $1}')
  echo "ROOTLESSKIT_PID = $ROOTLESSKIT_PID"
  wait ${ROOTLESSKIT_PID}
fi
